 ...
 function pbmit_circle_progressbar() {
  ...
  -------------------------------------*/
    ...
    $(window).scroll(function(){
        ...
        if (scroll >= 130) sticky.addClass('sticky-header');
  ...
  $('.pbmit-circle-outer').each(function() {
   ...
   /*-------------------------------------
    ...
    -------------------------------------*/
//код изменён и перемещён в additional.js(бэк)
...
document.addEventListener("DOMContentLoaded", () => {
	function masonryBuild() {
	   jQuery('.pbmit-element-viewtype-masonry').each(function() {
		   ...
		   // init Masonry
		   var $grid = main_ele.find('.pbmit-element-posts-wrapper').masonry({
		   ...
		   window.mainMasonry=$grid;
		   main_ele.addClass('js-masonry-initialised');

		   $grid.imagesLoaded().progress( function() {
			   $grid.masonry('layout');
		   });
		   main_ele.imagesLoaded().progress( function() {
			   $grid.masonry('layout');
		   });
   ...
   /* function masonryReload() {
		window.mainMasonry.masonry('destroy')
	   masonryBuild()
	}*/
   masonryBuild();
...
document.addEventListener("DOMContentLoaded", () => {
	...
	if(mIntroSlider) {
		let mIntroSliderEx = new Swiper(mIntroSlider, {
			slidesPerView: 1,
			effect: 'fade',
			pagination: {
				el: '.swiper-pagination',
	...
	const mImageSliders = document.querySelectorAll('[data-js="mImageSlider"]');

	if(mImageSliders.length > 0) {

		mImageSliders.forEach(mImageSlider => {
			let mImageSliderEx = new Swiper(mImageSlider, {
				slidesPerView: 1,
				spaceBetween: 24,
				navigation: {
					nextEl: '.swiper-button-next',
